...
export function pageFacts(html) {
  ...
  visit(parse(html));
  return { ids, links, headings, duplicates };
}
...
if (import.meta.main) {
  ...
  if (result.errors.length) throw new Error(`Documentation links failed:\n- ${result.errors.join('\n- ')}`);
  console.log(`Validated ${result.count} HTML pages, local links/assets, fragments, and ${Object.keys(legacy).length} legacy routes.`);
}
