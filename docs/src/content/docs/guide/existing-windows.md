---
title: "Adopting an existing Windows installation"
description: "Adopting an existing Windows installation for Winenv, the Windows 11 software management layer."
sidebar:
  order: 4
---

<span id="adopting-an-existing-windows-installation"></span>
...
`win diff` compares the effective profile with the current installed state and changes nothing. It reports missing packages, incompatible versions, different sources, unavailable managers, and declarations that need manual inspection. Software outside the profile is not considered drift; keep using `win scan` for the complete machine inventory. See [Composing profiles](/winenv/guide/profiles/#understand-win-diff) for the full status reference.
