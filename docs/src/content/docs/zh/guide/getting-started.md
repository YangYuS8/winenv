...
英文是项目的权威语言。第一次运行时，Winenv 会读取当前 Windows UI culture：`zh-*` 自动使用简体中文，其余语言使用英文。

```powershell
win lang             # 查看当前语言及其来源
win lang en          # 持久使用英文
win lang zh          # 持久使用简体中文
win lang auto        # 恢复系统自动检测
```

单次执行可以使用 `-Lang en`、`-Lang zh` 或 `-Lang auto`。`WINENV_LANG` 环境变量可为当前终端或自动化设置默认值；明确传入的 `-Lang` 优先级最高。
...
Profile 会作为独立的一层加入，不会覆盖内置运行层或其他用户 Profile。详见[组合 Profile](/winenv/zh/guide/profiles/)。
...
- 新电脑：直接[查找与管理软件](/winenv/zh/guide/packages/)。
- 已经使用一段时间的电脑：先[扫描并接入现有系统](/winenv/zh/guide/existing-windows/)。
- 有自己的软件基线：创建或导入[用户 Profile](/winenv/zh/guide/profiles/)。
