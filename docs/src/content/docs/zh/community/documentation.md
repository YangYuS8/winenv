...
中文加粗标签紧接正文时，把标点放在加粗标记外：写成 `**Scoop**：运行`，不要写成 `**Scoop：**运行`。检查渲染后的列表是否真的生成加粗元素，而不是显示星号。需要展示 Markdown 原文时，使用行内代码或围栏代码块。
...
构建检查页面元数据、JSON 示例、翻译配对、HTML 站内链接、资源和锚点，还会阻止渲染正文中残留成对的 `**` 标记，代码示例除外。它不会执行安装示例、发现所有格式错误，也不保证外部网站可用。英文修改需要在同一组变更中更新中文对应页；应复核并完成对应修改，而不是更新无意义的时间戳。语义准确性仍需要人工评审。
...
pnpm 初始化 Action 使用独立且锁定完整性校验的 npm 引导包。只在该步骤关闭 npm 的隐式漏洞查询，避免初始化被该接口阻塞；项目显式执行的签名检查不受影响。签名验证不等于漏洞扫描。
