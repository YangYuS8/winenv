...
- For maintainer-approved implementation tasks in this repository, routine delivery is delegated: choose coherent commits, push the work, merge validated changes into `main` through the permitted workflow, and follow the existing CI/Release/Pages automation to completion. Do not ask for separate permission at each step or stop at a green feature branch unless the user explicitly requests local-only/branch-only delivery or a genuine blocker remains.
- This delivery delegation does not authorize unrelated changes, repository/security-setting changes, bypassing required checks, or rewriting shared history. Preserve unrelated work. Pause for material requirement conflicts, scope expansion, or new risks as described above; do not confuse routine delivery choices with those decisions.
